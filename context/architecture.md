...
Important execution fields include the trade's `intent` (OPEN_LONG/OPEN_SHORT/CLOSE_LONG/CLOSE_SHORT) and provenance — either a `decision_id` (agent-initiated) or a `trigger_reason` (`stop_loss`/`take_profit`/`collateral_exhausted`/`profit_giveback` — the last added 2026-09-23, automatic, never both absent, and an agent-initiated close carries both `decision_id` and `trigger_reason = 'agent_close'`, not one or the other), accepted/rejected/clamped status, rejection reason, fill price, quantity, fee, slippage, and resulting portfolio state.

`positions` gained, 2026-09-23 (Strategy Profiles / Aggressive V3.1 — see the Strategy Profiles section below): the immutable ruler (`initial_entry_price`, `initial_stop_loss_price`, `initial_risk_usd`), `partial_realized_pnl_usd`, monitor-sampled high-water state (`sampled_mfe_r`, `sampled_mae_r`, `peak_total_pnl_usd`, `peak_pnl_at`, `giveback_floor_r`), and the eligibility gate `high_water_tracked_from`. `agent_decisions` gained the same-era `position_pnl_r`, `price_r`, `sampled_mfe_r`, `giveback_r`, `minutes_since_entry`, and `action_normalization_reason` (why a proposed action differs from the final one, when that's not a plain gate rejection).
...
4. Evaluate SL/TP/collateral-exhaustion triggers; where a window shows both SL and TP breached, SL wins. A position that breaches here never reaches step 4a below — SL/TP always wins the race, unchanged.
4a. **Aggressive V3.1 profit recycling (2026-09-23):** for a position that did NOT breach SL/TP this tick and whose `high_water_tracked_from` is set (never true for a position predating this migration — see the guarded backfill in Strategy Profiles below), `position-monitor/giveback.ts`'s `trackGivebackForPosition` replays the SAME points to advance `sampled_mfe_r`/`sampled_mae_r`/`giveback_floor_r`/`peak_total_pnl_usd`, walking chronologically and stopping at the first point (if any) where `positionPnlR` retraces to or below the armed floor. Sampling runs **regardless of the currently active strategy profile** (pure telemetry under Balanced); the resulting exit is only ever **executed** — closing the position, `close_reason = 'profit_giveback'` — when Aggressive is the active profile at read time. One atomic `.update()` per still-open, eligible position per tick, however many points were replayed.
5. Execute through the shared paper broker, using a conditional `UPDATE ... WHERE status = 'open' AND (quantity = $expected OR $expected IS NULL)` inside the fill transaction so a concurrent agent-cycle close — or, for a giveback close specifically, a same-tick ADD/REDUCE that changed the position's quantity after the ratchet's trigger was computed — is detected rather than double-executed or acted on with stale economics (§6 of the contract doc; the quantity guard is new in the giveback-close path only, every SL/TP close passes `$expected = NULL`).
6. Persist the trade, position close (or giveback close), and NAV snapshot.
...
- **Management:** a Choice (the action, with confidence and a full probability distribution) plus two speculative Scores (ADD/REDUCE magnitude — mapped to a code-defined fraction table, e.g. `{0.25, 0.50, 1.00}` for ADD, never a raw number) and two Choices (stop/target intent: `KEEP`/`TIGHTEN_TOWARD_ENTRY` — renamed 2026-09-23 from `TIGHTEN_TO_BREAKEVEN`, since it lands slightly inside entry, not at true breakeven; `KEEP`/`MOVE_CLOSER`/`MOVE_OUT`). **`MODIFY_PROTECTION` itself is omitted from the offered action set whenever no legal tighten exists** (`model/jev/management-question.ts`'s `isProtectionActionable`, 2026-09-23) — previously an underwater position's only coherent stop-tighten answer would have been silently rejected by the gate (a disguised CLOSE), so the option is removed at the source instead of offered-then-rejected. `cycle/apply-management.ts` is the single place an accepted intent turns into a proposal — for `MODIFY_PROTECTION` specifically, this is the "deterministic code computes the price" step (one ATR-in-price-terms move for the target; the tightest legal stop just inside entry for the tighten), which the risk gate then validates, never trusts. **Aggressive-only, additionally:** the action question is reframed around marginal return (profit already accumulated vs. remaining expected upside) rather than "are you bullish?", quoting `positionPnlR`/`sampledMfeR`/`givebackRatio`/`priceR`/`costR` (`buildActionInstructions`) — Balanced's wording is unchanged. A sixth question, `{asset}_remaining_upside` (Aggressive-only), reuses the entry path's expected-move table so `expected_move_pct`/`move_to_cost_ratio` are finally populated on management rows too.
...
**New 2026-09-23.** `agent_settings.strategy_profile` (`'balanced' | 'aggressive'`, fails closed — `'conservative'` is not a member) selects which strategy originates decisions for every asset, read fresh by `agent-cycle` every cycle — no in-memory cache, no retroactive effect on an already-open position. `strategy/registry.ts` is the one dispatch point: a set of small functions taking `profile` as their own first argument (`checkStrategyDataSufficiency`, `regimeContextFor`, `managementAtrPctFor`, `protectionForEntry`, `clearsEntryTradeabilityFloor`), not a class hierarchy.

**Balanced** is exactly the Agent Cycle / Model Boundary / Risk Gate flow described above, unmodified — `strategy_version: 'v1-regime'`.

**Aggressive** (`strategy_version: 'v3.1-jev-intraday-30m'` — a 15-minute decision cadence over a 30-minute signal, not a "15-minute strategy"; preserve this distinction in any new code) replaces entry origination and adds a monitor-enforced profit-protection layer on top of the shared risk gate / paper broker / persistence path, which otherwise applies identically to both profiles:

- **Data sufficiency** (`checkStrategyDataSufficiency`): Balanced's 50-daily-bar rule never gates Aggressive; Aggressive instead requires ≥12 closed 30-minute OHLC bars and ≥25 closed 5-minute spot points (`providers/coingecko.ts`'s `fetchIntradayMarketData`). The 50-day regime stays available as context for Aggressive (`regimeContextFor`), never a hard gate.
- **Entry origination** (`strategy/aggressive/detectors.ts`): two deterministic, edge-triggered detectors on closed 30-minute bars — `MOMENTUM_BREAKOUT` (close exceeds the prior 8-bar high) and `PULLBACK_CONTINUATION` (a frozen wick-based formula). Edge-triggered means the predicate must be false on the bar immediately before and true on the current one — a condition that merely remains true never re-fires, and neither does a candidate Jev declines. A candidate is emitted at all only if it clears the tradeability floor (`clearsTradeabilityFloor`, K=3: `atrTargetDistancePct >= 3 × estimatedRoundTripCostPct`) — below this, a 4×ATR target would be nearly consumed by round-trip cost before any edge exists.
- **Entry judgement, never origination** (`model/jev/entry-question.ts`): for a FLAT asset with a live opportunity, the batched request additionally carries `{asset}_entry_quality` (choice: ENTER/SKIP) and `{asset}_expected_move` (score, mapped through a pre-registered table). Jev can only ever REMOVE a detected candidate (veto, or SKIP) — it cannot invent one where no detector fired.
- **Protection at origination** (`strategy/aggressive/protection.ts`'s `aggressiveProtectionFor`): `stopLossPct = max(2×ATR30, 0.8%)`, `takeProfitPct = 4×ATR30` — NOT Balanced's 2.5%-floor formula, and explicitly not a constant 2R (only true while the ATR term dominates the 0.8% floor). `protectionForEntry` is called ONLY at origination, never against an already-open position — a profile switch never recomputes an existing position's SL/TP.
- **Management** reuses the shared Phase 2 flow (HOLD/ADD/REDUCE/CLOSE/MODIFY_PROTECTION) with Aggressive-specific context and question wording layered on top — see the next section.
- **Capital policy**: Aggressive's own `riskBudgetPct`/`maxSingleTradePct`/`maxTotalNotionalPct`/`stopOutReentryBlockMinutes` (`src/shared/strategy/profiles.ts`), reachable via raised DB ceilings that leave Balanced's own (smaller) effective caps unchanged (`min(profile value, ceiling)`).

### Aggressive V3.1 — profit recycling and the giveback ratchet

The first live Aggressive observation showed the original design's profit-protection mechanism was structurally inert: its profit-locking stop rung computed a stop-loss price *above* entry for a long, which `positions_sl_tp_ordering_valid` makes impossible to persist (confirmed zero executions across the project's history), and R was sampled only at manual decision cycles, so real intraday peaks were routinely missed. Full diagnosis and design: `context/specs/trading-strategy-aggressive-v3.md`.

**Two R metrics, computed against the same immutable ruler but never conflated:**

- `priceR` — market-path distance from the ORIGINAL entry (`positions.initial_entry_price`/`initial_stop_loss_price`), in original-risk units. Context only, never read as profit.
- `positionPnlR` — `(unrealizedPnlUsd + partialRealizedPnlUsd) / initialRiskUsd`. THE profit metric. `partialRealizedPnlUsd` (`positions.partial_realized_pnl_usd`, accumulated by every REDUCE) is what keeps this continuous across a partial exit — without it, a deliberate harvest would misreport as giveback.

The immutable ruler (`initial_entry_price`/`initial_stop_loss_price`/`initial_risk_usd`) is captured once, atomically, inside `open_position_atomic` for EVERY new position (both profiles — Balanced never reads it) and never redefined by a later ADD/REDUCE. A position opened before this migration is backfilled the ruler only if no protection change was ever recorded against it (structurally verified, not assumed); `high_water_tracked_from` — the sole eligibility gate for the giveback mechanism — is deliberately left NULL for every backfilled position, since its true historical peak predates any tracking and sampling it now would understate MFE.

**The giveback ratchet** (`strategy/aggressive/protection.ts`'s `rawGivebackFloor`/`nextGivebackFloor`/`shouldExecuteGivebackExit`, orchestrated by `position-monitor/giveback.ts`) arms once `sampledMfeR >= 1.0` and ratchets a monotone floor (never decreases) as the position's best-ever `positionPnlR` rises further: `costR` at +1R, `0.5R` at +1.5R MFE, `1.0R` at +2R MFE, `1.5R` at +3R MFE. The position closes (`close_reason: 'profit_giveback'`) the instant `positionPnlR` retraces to or below the armed floor. Sampling (MFE/MAE/floor) runs every monitor tick regardless of the active profile; the EXIT is Aggressive-only, checked at execution time — switching to Balanced disables the exit immediately without touching any SL/TP, and switching back to Aggressive resumes with intact high-water history rather than a reset ratchet.
